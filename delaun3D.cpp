...
/*	ofstream nod("node.dat",ios::app);
	...
	while(num_ON!=0)
	{
		...
		for(int i=1;i<=node;i++)
		{   
			if(flag[i]==ON && (NODE[i].material==MAGELAST || NODE[i].material==FLUID))
			{
				int ip=i;
				double xp=NODE[ip].r[A_X];//導入する節点の座標
				double yp=NODE[ip].r[A_Y];
...
void set_material(mpsconfig &CON,vector<point3D> &NODE,vector<element3D> &ELEM,int node,int nelm)
{
    for(int i=1;i<=nelm;i++)
    {
		...
		{
			if(M1!=ELASTIC && M2!=ELASTIC && M3!=ELASTIC && M4!=ELASTIC) ELEM[i].material=MAGELAST;
	...
	for(int i=1;i<=nelm;i++)
	{
		if(ELEM[i].material!=AIR)
		{
			for(int j=1;j<=4;j++)
			{
				...
				{
					if(ELEM[i].material==FLUID ||  ELEM[i].material==MAGELAST || ELEM[i].material==MAGELAST2)
					{
						red   = 0.0;
						green = 1.0;
						blue  = 0.0;
...
int make_air_layer(vector<point3D> &NODE,vector<element3D> &ELEM,int *nelm,mpsconfig &CON,int *node_num,int KTE,double rrm,int KTJ)
{
	...
	{
		if(ELEM[je].material==FLUID || ELEM[je].material==ELASTIC || ELEM[je].material==MAGELAST)
		{
			for(int j=1;j<=4;j++)
			{
	...
	for(int i=1;i<=nelm;i++)
	{
		if(ELEM[i].material==MAGNET || ELEM[i].material==COIL)//表示したい材質
		{
			for(int j=1;j<=4;j++)
			{
				...
				if(ELEM[jelm].material==AIR)
				{
					if(ELEM[i].material==FLUID || ELEM[i].material==ELASTIC || ELEM[i].material==MAGELAST)
					{
						red   = 0.0;
						green = 0.0;
						blue  = 1.0;
