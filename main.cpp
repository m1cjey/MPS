...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for(int i=0;i<PART.size();i++)
	{
		...
		for(int i=0;i<particle_number;i++)
		{
			...
			PART[i].q0[A_X]=PART[i].r[A_X];
			PART[i].q0[A_Y]=PART[i].r[A_Y];
			PART[i].q0[A_Z]=PART[i].r[A_Z];
	...
	{	
		...
		if(t==1)
		{
			ofstream fs0("stress258.csv", ios::trunc);
			fs0.close();
		}
		ofstream fs1("stress258.csv", ios::app);
		fs1<<PART[258].volumetric_strain_z<<",";
		fs1<<PART[258].get_stress_accel(A_Z)<<endl;
		fs1.close();
